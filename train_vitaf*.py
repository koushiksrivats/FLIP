...
from utils.evaluate import eval_ViTAF
from utils.dataset import get_dataset
from utils.dataset import get_dataset_one_to_one
from fas import fas_model_adapt, fas_model_fix_vitaf
import random
import numpy as np
...
def train(config):
  ...
  net1 = fas_model_fix_vitaf().to(device)
  optimizer_dict = [
      {
  ...
  for name, param in net2.named_parameters():
    ...
    elif name.find('embedder') != -1 or name.find('classifier') != -1:
        param.requires_grad = True
    else:
  ...
  for iter_num in range(4000 + 1):
    ...
    if (iter_num != 0 and (iter_num + 1) % (iter_per_epoch) == 0):
      valid_args = eval_ViTAF(test_dataloader, net2, True)
      ...
      save_checkpoint(save_list, is_best, net2,
                      os.path.join(config.op_dir , config.tgt_data + f'_vitaf*_5_shot_checkpoint_run_{str(config.run)}.pth.tar'))
