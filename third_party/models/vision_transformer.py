...
class Block(nn.Module):
  ...
  def __init__(self,
    ...
    self.gamma = torch.nn.Parameter(torch.ones(1, 1, num_features) * gamma)
    self.beta = torch.nn.Parameter(torch.ones(1, 1, num_features) * beta)
...
class VisionTransformer(nn.Module):
  ...
  def forward_features(self, x):
    ...
    x, total_loss = self.blocks(x)
    x = self.norm(x)
    if self.dist_token is None:
      return self.pre_logits(x[:, 0]), total_loss
    else:
      return x[:, 0], x[:, 1]
...
@register_model
def vit_base_patch16_224(gamma=0.3, beta=0.5, pretrained=False, **kwargs):
